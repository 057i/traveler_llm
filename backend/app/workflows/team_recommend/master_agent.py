...
from .subagents.query_rewriter import QueryRewriter
...
class MasterAgent:
    ...
    def __init__(self):
        ...
        # Initialize subagents（添加Query重写器）
...
请整合以上信息，生成简洁的行程规划方案。用中文回答。"""
    ...
    async def _run_rag_assistant(self, query: str) -> List[Dict[str, Any]]:
        """执行RAG助手（使用工具）"""
        try:
            logger.info("[Master] Calling RAG Assistant...")

            # 提取实体（简单版）
            entities = self._extract_entities(query)

            # 调用新的search方法
            result = await self.rag_assistant.search(
                query=query,
                entities=entities
            )

            rag_results = result.get('top_results', [])
            confidence = result.get('confidence', 0.0)

            logger.info(f"[Master] RAG Assistant returned {len(rag_results)} results")
    ...
    async def _run_graph_rag_assistant(
        ...
        """执行图RAG助手（使用工具）"""
        try:
            logger.info("[Master] Calling Graph RAG Assistant...")
            ...
            # 调用新的search方法
            result = await self.graph_rag_assistant.search(entities=entities)

            graph_results = result.get('results', [])

            logger.info(f"[Master] Graph RAG Assistant returned {len(graph_results)} results")
    ...
    def _extract_entities(self, query: str) -> List[str]:
        """简单的实体提取（从查询中）"""
        # TODO: 可以使用NER模型，这里先用简单规则
        import re

        # 常见景点关键词
        keywords = ['山', '寺', '湖', '岛', '古镇', '公园', '景区', '风景区']

        entities = []
        for keyword in keywords:
            pattern = f'[\\u4e00-\\u9fa5]+{keyword}'
        ...
        logger.info(f"[Master] Extracted entities: {entities}")
